...
keyboard:
  tab, arrow keys or Ctl+tab: |
    Focus visibly moves to the tooltip
  spacebar: |
...
screenreader: 
  name:  |
    Purpose is clear and matches visible label (if any)
  role:  |
    Identifies as a button in iOS and _component or "double tap to activate" in Android
  group: |
    n/a
  state: |
    n/a
...
-->
## Developer notes
- Name and Role must be stated in a single announcement when focus is on the control
- Touch target for each tooltip should meet a minimum of 48 x 48px
- Tooltip is also often called a help tag or TipKit.
...
- When a tooltip is closed, the focus should return to the triggering element.
...
- **UIKit**
  - If VoiceOver is not reaching a particular element, set the element's `isAccessibilityElement` to `true`
    - **Note:** You may need to adjust the programmatic name, role, and/or value after doing this, as this action may overwrite previously configured accessibility.
